// Motor_Test.c main code for Gantry Crane PID Control
// ENGPHYS 3BB3 Final Design Project
// Jarod Coppens, Josh Eizenga - March 2019

//-----------------------------------------------------------------
//  Read Me
//-----------------------------------------------------------------
//  CranePID_v1.c code for ENGPHYS 3BB3 final PID project
...
int position = 0, i = 0;
int KPX = 8,  KDX = 4, KIX =1;
int KPA = 150, KDA = 100; 
int targetX = 2;   //target position (updated by MATLAB input)
...
// Motor Control
//-----------------------------------------------
// motor moves in "forward" direction with duty cycle of N 
void motorForward(int duty){ 
  TA1CCR1 = duty;
  IN2 = 0;
}
// motor moves in "reverse" direction with duty cycle of N 
...
void main(){
  ...
  while(1){
    ...
    derivativeA = 0.3*derivativeA + 0.7*(angle - prevAngle);
    
    output = -angleError*KPA - (int)derivativeA*KDA + positionError*KPX + derivativeX*KDX + integralX*KIX;
